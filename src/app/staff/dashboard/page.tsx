...
import { ENABLE_TABLE_ASSIGNMENTS } from '@/lib/staffConfig'
...
export default function StaffDashboard() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(tablesRef, (snapshot) => {
      ...
      // If waiter, filter by assigned tables if feature is enabled
      if (user.role === 'waiter' && ENABLE_TABLE_ASSIGNMENTS) {
        setTables(tablesData.filter(t => user.assignedTables?.includes(t.id)))
      } else {
    ...
    return () => unsubscribe()
  }, [user?.restaurantId, user?.role, user?.id, ENABLE_TABLE_ASSIGNMENTS])

  // Subscribe to selected table's active session
  useEffect(() => {
    if (!user?.restaurantId || !selectedTable) return

    // If waiter, ensure the table belongs to them if feature is enabled
    if (user.role === 'waiter' && ENABLE_TABLE_ASSIGNMENTS && !user.assignedTables?.includes(selectedTable.id)) {
    ...
    return () => unsubscribe()
  }, [user?.restaurantId, selectedTable, user?.role, user?.id, ENABLE_TABLE_ASSIGNMENTS])
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToOrders(user.restaurantId, (allOrders) => {
      ...
      let active = allOrders.filter(o => {
        const status = (o.status || '').toUpperCase();
        return ['PENDING', 'ACCEPTED', 'PREPARING', 'READY', 'SERVED', 'BILL_GENERATED', 'PAID'].includes(status);
      })
      
      // If waiter, filter by assigned tables if feature is enabled
      if (user.role === 'waiter' && ENABLE_TABLE_ASSIGNMENTS) {
        active = active.filter(o => tables.some(t => t.id === o.tableId));
      }
    ...
    return () => unsubscribe()
  }, [user?.restaurantId, user?.role, tables, ENABLE_TABLE_ASSIGNMENTS])
